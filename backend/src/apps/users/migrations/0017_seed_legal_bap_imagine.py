...
def seed_legal_bap_imagine(apps, schema_editor):
...
def unseed(apps, schema_editor):
    LegalDocumentVersion = apps.get_model('users', 'LegalDocumentVersion')
    LegalDocumentVersion.objects.filter(version='2026.08.04').delete()
    for doc_type, prev in (('T', '2026.07.26'), ('P', '2026.07.26')):
        prev_doc = (
...
class Migration(migrations.Migration):
    dependencies = [
        ('users', '0016_seed_legal_fees_settlement'),
    ]

    operations = [
        migrations.RunPython(seed_legal_bap_imagine, unseed),
    ]
