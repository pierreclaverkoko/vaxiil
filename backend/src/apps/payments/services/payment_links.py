...
def _default_provider() -> PaymentProvider:
    """MainMoney is the only supported checkout provider for payment links."""
        ...
        raise ValidationError(
            {
                'payment_provider': gettext(
                    'MainMoney payment is not configured. '
        ...
        if apply_wallet or (wallet_amount is not None and wallet_amount > 0):
            ...
            if wallet_amount is None:
                ...
                if to_apply > available:
                    raise ValidationError(
                        {'wallet_amount': gettext('Insufficient refund wallet balance.')}
                    )
